...
| Path | Purpose |
|---|---|
| `sysops/signoz/casting.yaml` | Foundry installation spec: pinned image digests and platform patches |
| `sysops/signoz/casting.yaml.lock` | Committed forge output; CI fails on lock drift |
| `sysops/signoz/deploy/deploy` | Host-side delivery: pinned Foundry install, cast, health wait |
| `sysops/signoz/pours/` | Generated compose output — gitignored, never edited |
...
| Path | Purpose |
|---|---|
| `sysops/signoz/collector/config.yaml` | Receivers, OTTL redaction, and exporters |
| `sysops/signoz/collector/compose.yaml` | Hardened agent runtime (0.25 CPU / 512M, ro mounts) |
| `sysops/signoz/collector/image` | Reviewed digest-pinned contrib image |
| `sysops/signoz/collector/deploy` | Host-side validate-and-run script |
...
- Container logs through `receiver_creator` + `docker_observer`: resource attributes
  ...
  ArXiv Lens, metadata-postgres, cloudflare). Ephemeral dbt/OCR task containers, SigNoz's own
...
Dashboards and alert rules exist only as Terraform (`sysops/signoz/terraform/`), managed by
