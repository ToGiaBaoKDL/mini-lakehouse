...
resource "aws_kms_key" "lakehouse" {
  description             = "${var.name_prefix}-${var.environment} lakehouse data"
...
resource "aws_kms_alias" "lakehouse" {
  name          = "alias/${var.name_prefix}-${var.environment}-lakehouse"
  target_key_id = aws_kms_key.lakehouse.key_id
}

resource "aws_s3_bucket" "this" {
  for_each      = var.bucket_tiers
  bucket        = "${var.name_prefix}-${var.environment}-${each.key}-${random_string.suffix[each.key].result}"
