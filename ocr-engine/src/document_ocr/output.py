...
def validate_ocr_output(
    ...
    *,
    ...
    try:
        result = OcrOutput.model_validate_json((directory / OCR_RESULT_FILE).read_bytes())
        ...
        if expected_job_id is not None and result.job_id != expected_job_id:
            raise ValueError(f"expected job {expected_job_id}, received {result.job_id}")
...
def extract_ocr_output(
    ...
    *,
    ...
    if output.document_id != job.document_id:
        raise InvalidOcrArchiveError("OCR result does not match its job")
    if output.processing_id != processing_id(
    ...
    if output.state == "reused":
        reuse = job.reuse
        if reuse is None or (
            output.pdf_sha256,
            output.pdf_size_bytes,
            output.page_count,
        ...
        and job.reuse is not None
        and output.pdf_sha256 == job.reuse.pdf_sha256
    ):
    ...
    document_manifest = output.manifest
    expected_paths = (
