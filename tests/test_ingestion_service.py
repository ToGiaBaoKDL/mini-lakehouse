...
from mini_lakehouse.storage.iceberg import LandingEventsRepository, LandingHourWrite
...
def test_ingestion_skips_download_and_parse_when_hour_is_complete() -> None:
    ...
    repository = create_autospec(LandingEventsRepository, instance=True)
    object_store.exists.return_value = True
    repository.hour_state.return_value = LandingHourWrite(
        row_count=42,
        snapshot_id=7,
        was_appended=False,
    )
    ...
    assert result.was_appended is False
    client.download.assert_not_called()
    object_store.download.assert_not_called()
    repository.append_hour.assert_not_called()
